...
class DisplayWaitPhamacy extends CI_Controller {
    ...
    public function index(){
        ...
        AND  r.`rx_depcode` IN($qdep) -- in('030','082')
        ...
        $sql2 = $this->db->query("SELECT v.vn,v.rx_queue,v.hn,v.pt_priority,pty.pcode,v.oqueue,(vns.rcpt_money-vns.paid_money) AS pmoney,
        v.cur_dep_time,q2.stock_department_queue_no,COUNT(s.vn) AS svn_count,    COUNT(r.vn) AS rx_count,COUNT(r1.vn) AS pay_count  ,
        COUNT(t.vn) AS finance_count,COUNT(r2.vn) AS rx_print_count   , k.department  , CONCAT(p.pname,p.fname,'  ',p.lname) AS ptname
        ,COUNT(op.icode)AS cc  
        FROM ovst v 
        ...
        $this->load->view('DisplayWaitPhamacy_view1',$data);
