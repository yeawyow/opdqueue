...
var SubQ = '';
var DrugQ = '';
conq.getConnection(function(err, cnn) {
    cnn.query("SELECT call_queue,sub_queue,drug_queue FROM `sys_configs`", function (err, result, fields) {
...
io.on('connection', function(socket){
    ...
    socket.on('HOSxPScreen', function (data) {
        ...
        if(CallQueue == 'Y'&& SubQ=='N'){
            fs.exists('audio/'+queue+'.mp3', function(exists) {
                if(exists==false){
                    const tts = new gTTS('เชิญหมายเลข'+queue,'th');
                    tts.save('audio/'+hn+'.mp3', function (err, result) {
                            ...
                            if (fs.existsSync('audio/'+hn+'.mp3')) {
                                ...
                                pool.getConnection(function(err, con) {
                                    ...
                                    con.query(ptname,[hn], function(err, rows) {
                                        ...
                                        Object.keys(rows).forEach(function(key) {
                                            var row = rows[key];
                                            const tts = new gTTS(row.ptname,'th');
                                            tts.save('audio/'+hn+'.mp3', function (err, result) {
                                                if(err) { throw new Error(err) }
                                                console.log('เปลี่ยนชื่อโหลดไฟล์ใหม่  mp3',hn);
                                                io.sockets.emit( 'hn', { 
                                ...
                                con.query(ptRename,[hn], function(err, result) {
                                    ...
                                    Object.keys(result).forEach(function(key) {
                                        ...
                                        fs.exists('audio/'+hn+'.mp3', function(exists) {
                                            ...
                                            if(exists==false){
                                                const tts = new gTTS(row.pname,'th');
                                                console.log('row.pname : ',row.pname)
                                                // tts.save('audio/'+hn+'.mp3', function (err, result) {
                                                //     if(err) { throw new Error(err) }
                                                //     console.log('Success! ');
                                                // });
        ...
        if(CallQueue == 'Y' && DrugQ == '1'){
            fs.exists('audio/'+queue+'.mp3', function(exists) {
                if(exists==false){
                    ...
                    const tts = new gTTS('เชิญหมายเลข'+queue,'th');
                    // tts.save('audio/'+queue+'.mp3')
                    tts.save('audio/'+queue+'.mp3', function (err, result) {
                        if(err) { throw new Error(err) }
            ...
            fs.exists('audio/'+rx_queue+'.mp3', function(exists) {
                if(exists==false){
                    const tts = new gTTS('เชิญหมายเลข'+rx_queue,'th');
                    tts.save('audio/'+rx_queue+'.mp3', function (err, result) {
                        if(err) { throw new Error(err) }
    ...
    // start PhamaIPD
    socket.on('PhamaIPD', function (data) {
            ...
            fs.exists('audio/'+queue+'.mp3', function(exists) {
                if(exists==false){
                    ...
                    tts.save('audio/'+queue+'.mp3', function (err, result) {
                        if(err) { throw new Error(err) }
                        console.log('load Queue number mp3',queue);
                        io.sockets.emit( 'call_phama_ipd_room', { 
                            hn:hn,
                            Queue:queue,
                            chanel:station,
                            CallQueue:'Y'
    ...
    socket.on('ExitCare', function (data) {
        console.log('ExitCare :',data.hosxp.split('\n')[3])
        var a = data.hosxp;
        var b = a.split('\n');
        var c = b.pop();
        var d = c.split(';');
        ...
        if (fs.existsSync('audio/'+hn+'.mp3')) {
            ...
            pool.getConnection(function(err, con) {
                con.query("SET NAMES UTF8");
                con.query(q,[hn], function(err, rows) {
                    if (err) throw err;
                    Object.keys(rows).forEach(function(key) {
                        var row = rows[key];
                        const tts = new gTTS(row.ptname,'th');
                        tts.save('audio/'+hn+'.mp3')
                            .then(() => {
                                console.log('ExitCareCall โหลดไฟล์ใหม่ ',hn);
                                io.sockets.emit( 'ExitCareCall', { 
                                    HN:hn,
                                    Queue:queue,
                                    station:station
                            ...
                            .catch((err) => {
                                console.log('failed ExitCareCall โหลดไฟล์ใหม่ :'+hn);
                            });
                    }); 
                    //console.log(rows); 
                }); 
    ...
    /** เรียกเข้าห้องตรวจแพทย์ */
    socket.on( 'call_to_exam_room', function( dataD ) {
        //console.log('HOSxP Send :',data.hosxp.split('\n')[3])
        const dataOfLen = Object.getOwnPropertyNames(dataD);
        if( dataOfLen.length == 1 ){
            var a = dataD.hosxp;
            var b = a.split('\n');
            var c = b.pop();
            var d = c.split(';');
            ...
            var hn = h.substring(0,h.length-1);
            var s = d[4].split('=')[1].substring(2); 
            var roomno = s.substring(0,s.length-1);
        }else{
            var hn = dataD.HN;
            var queue = dataD.queue;
            var roomno = dataD.roomno;
        }
            ...
            pool.getConnection(function(err, con) {
                ...
                con.query(q,[hn], function(err, rows) {
                    ...
                    Object.keys(rows).forEach(function(key) {
                        ...
                        tts.save('audio/'+hn+'.mp3')
                            .then(() => {
                                console.log('call_to_exam_room โหลดไฟล์ใหม่ ',hn);
                                io.sockets.emit( 'call_to_exam_room', { 
                                    HN:hn,
                                    Queue:queue,
                                    station:roomno
                            ...
                            .catch((err) => {
                                console.log('failed call_to_exam_room โหลดไฟล์ใหม่ :'+hn);
                            });
                    }); 
                    //console.log(rows); 
                }); 
    ...
    socket.on( 'Dent', function( dataD ) {
        ...
        var q = 'select concat("เชิญคุณ"," ",fname," ",lname)as ptname from patient where hn=?'
        if (fs.existsSync('audio/'+hn+'.mp3')) {
            console.log('ห้องฟันมีไฟล์แล้ว');
            io.sockets.emit( 'Dent', { 
                HN:hn,
                Queue:queue,
                station:roomno
            });
            ...
            pool.getConnection(function(err, con) {
                ...
                con.query(q,[hn], function(err, rows) {
                    ...
                    Object.keys(rows).forEach(function(key) {
                        ...
                        tts.save('audio/'+hn+'.mp3')
                            .then(() => {
                                console.log('Dent โหลดไฟล์ใหม่ ',hn);
                                io.sockets.emit( 'Dent', { 
                                    HN:hn,
                                    Queue:queue,
                                    station:roomno
                            ...
                            .catch((err) => {
                                console.log('failed Dent โหลดไฟล์ใหม่ :'+hn);
                            });
                    }); 
                    //console.log(rows); 
                }); 
    ...
    socket.on('loadFile', function( data ) {
        ...
        if(data.number=='num'){
            for(var i=0;i<=1000;i++){
                const tts = new gTTS('เชิญหมายเลข'+i,'th');
    ...
    socket.on('systemtray_click', function( data ) {
        console.log('systemtray_click data :',data);
        ...
        conq.getConnection(function(err, cnn) {
            sql = "REPLACE INTO `sys_systemtray`(`vn`,`tblrows`,`tbl`,`tblcall`)VALUES(?,?,?,?)";
            

            cnn.query(sql,[data.vn,data.row,data.tbl,tblcall], function (err, result, fields) {
                // if any error while executing above query, throw error
                if (err) throw err;
        ...
        io.sockets.emit( 'systemtray_click', { 
            hn:data.hn,
            row:data.row,
            tbl:data.tbl
        });
    ...
    socket.on('systemtray_skip', function( data ) {
        console.log('systemtray_skip :',data);

        conq.getConnection(function(err, cnn) {
            sql = "REPLACE INTO `snk_call_skip`(`vn`,msg)VALUES(?,?)";
            cnn.query(sql,[data.vn,data.msg], function (err, result, fields) {
                // if any error while executing above query, throw error
                if (err) throw err;
                console.log('insert systemtray_skip');
            });
            cnn.release();
        });

        io.sockets.emit( 'systemtray_skip', { 
            hn:data.hn,
            row:data.row,
            tbl:data.tbl
        });
